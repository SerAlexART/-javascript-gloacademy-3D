...
const sendData = ({ url, data = {} }) => {
    ...
    // * Представим, что мы функцию sendData мы будем использовать в разных частях нашего приложения и каждый раз мы будем раскрывать response через метод json(), мы можем это универсилизировать. Просто передадим метод thex в конце функции и в ответ мы будем сразу получать уже раскрытыый response
};
